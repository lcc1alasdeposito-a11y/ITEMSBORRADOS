...
`);
    ...
    buildEstadoBadge = function(estado) {
        ...
        return '<span class="status-badge" style="background:' + s.bg + ';color:' + s.tc + ';font-weight:700;font-size:11px;padding:4px 10px;border-radius:6px;display:inline-flex;align-items:center;gap:4px">' + s.i + ' ' + s.nm + '</span>';
    },
    ROLES = {
        operario: {
            nm: "Operario"
    ...
    try {
        ...
        function inspectRows(rows) {
            (rows || []).forEach(function(row) {
                ...
                if (factKeys[key] && (row.estado === "contabilizado" || row.estado === "recuperado")) {
        ...
        for (var i = 0; i < docs.length; i += DOC_CHUNK) {
            ...
            for (var from = 0;; from += RANGE_SIZE) {
                var query = sup.from("items_borrados")
                    ...
                    .in("estado", ["recuperado", "contabilizado"])
            ...
            for (var gi = 0; gi < groupKeys.length; gi++) {
                ...
                for (var fi = 0; fi < groupIds.length; fi += batchSize) {
                    var factBatch = groupIds.slice(fi, fi + batchSize),
                        payload = { estado: estado, fecha_gestion: new Date().toISOString() };
...
async function db_importItems(items) {
    ...
    try {
        ...
        var { data: existing, error: err } = await supabase
            .from("items_borrados")
            .select("id, doc_vtas, fecha_carga, material")
            .in("doc_vtas", docList);
        if (err) throw err;
        var existingMap = {};
        (existing || []).forEach(function(r) {
            var f = (r.fecha_carga || "").substring(0, 10);
            existingMap[(r.doc_vtas || "") + "|" + (r.material || "") + "|" + f] = r.id
        });
        var toInsert = [], toUpdate = [];
        items.forEach(function(x) {
            var f = (x.fecha_carga || "").substring(0, 10);
            var key = (x.doc_vtas || "") + "|" + (x.material || "") + "|" + f;
            var existingId = existingMap[key];
            existingId ? toUpdate.push({ id: existingId, data: x }) : toInsert.push(x)
        });
        ...
        for (var q = 0; q < items.length; q++) try {
            ...
            if (exists) {
                ...
                var { error: insErr2 } = await supabase.from("items_borrados").insert(item);
...
async function db_getHoyItems(estado) {
    ...
    var q = sup.from("items_borrados").select("*").eq("estado", estado).gte(dateField, hoyStr + "T00:00:00").lte(dateField, hoyStr + "T23:59:59").order(dateField, { ascending: !1 });
...
async function db_getEstadoItems(estado, limit) {
    var sup = getSupabase(),
        size = limit || 500,
        dateField = estado === "facturado" ? "fecha_fact" : "fecha_gestion",
...
async function db_getStats(l, desde, hasta) {
    ...
    var d = function(q) {
        ...
        dhFact = function(q) {
            ...
            if (l) U = U.eq("vendedor_externo", l);
            U = U.gte("fecha_fact", hoyStr + "T00:00:00").lte("fecha_fact", hoyStr + "T23:59:59");
            return U
        },
        [g, b, k, E, I, R_M, N_M, C_M, F_M, hoyRCnt, hoyRM, hoyNoRCnt, hoyNoRM, hoyCCnt, hoyCM, hoyFCnt, hoyFM, conCnt, facCnt] = await Promise.all([d("count"), d("count").eq("estado", "pendiente"), d("count").eq("estado", "recuperado"), d("count").eq("estado", "no_recuperado"), d("total_importe"), d("total_importe").eq("estado", "recuperado"), d("total_importe").eq("estado", "no_recuperado"), d("total_importe").eq("estado", "contabilizado"), d("total_importe").eq("estado", "facturado"), dh("count").eq("estado", "recuperado"), dh("total_importe").eq("estado", "recuperado"), dh("count").eq("estado", "no_recuperado"), dh("total_importe").eq("estado", "no_recuperado"), dh("count").eq("estado", "contabilizado"), dh("total_importe").eq("estado", "contabilizado"), dhFact("count").eq("estado", "facturado"), dhFact("total_importe").eq("estado", "facturado"), d("count").eq("estado", "contabilizado"), d("count").eq("estado", "facturado")]);
    ...
    var $ = 0,
        ...
        hoyNoRM$ = 0,
        hoyC = hoyCCnt ? hoyCCnt.count : 0,
        hoyCM$ = 0,
        hoyF = hoyFCnt ? hoyFCnt.count : 0,
        hoyFM$ = 0,
        conCntVal = conCnt ? conCnt.count : 0,
        facCntVal = facCnt ? facCnt.count : 0;
    if (I.data)
    ...
    if (C_M && C_M.data)
        for (var L = 0; L < C_M.data.length; L++) {
            var w = Number(C_M.data[L].total_importe);
            isNaN(w) || (C_$ += w)
        }
    if (F_M && F_M.data)
        for (var L = 0; L < F_M.data.length; L++) {
            var w = Number(F_M.data[L].total_importe);
            isNaN(w) || (F_$ += w)
        }
    if (hoyRM && hoyRM.data)
    ...
    if (hoyCM && hoyCM.data)
        for (var L = 0; L < hoyCM.data.length; L++) {
            var w = Number(hoyCM.data[L].total_importe);
            isNaN(w) || (hoyCM$ += w)
        }
    if (hoyFM && hoyFM.data)
        for (var L = 0; L < hoyFM.data.length; L++) {
            var w = Number(hoyFM.data[L].total_importe);
            isNaN(w) || (hoyFM$ += w)
        }
    var flujoRecuperado = (k.count || 0) + conCntVal + facCntVal,
        montoFlujoRecuperado = R_$ + C_$ + F_$,
        pendienteFacturar = (k.count || 0) + conCntVal,
        montoPendienteFacturar = R_$ + C_$,
        hoyFlujoRecuperado = hoyR + hoyC + hoyF,
        hoyMontoFlujoRecuperado = hoyRM$ + hoyCM$ + hoyFM$,
        hoyPendienteFacturar = hoyR + hoyC,
        hoyMontoPendienteFacturar = hoyRM$ + hoyCM$;
    var v = {};
    try {
    ...
    return {
        ...
        pendiente: b.count,
        recuperado: k.count,
        recuperadoBase: k.count,
        no_recuperado: E.count,
        contabilizado: conCntVal,
        facturado: facCntVal,
        ...
        totalMonto: $,
        montoRecuperado: R_$,
        montoRecuperadoBase: R_$,
        montoNoRecuperado: N_$,
        montoContabilizado: C_$,
        montoFacturado: F_$,
        hoyRecuperados: hoyR,
        hoyRecuperadosBase: hoyR,
        hoyMontoRecuperado: hoyRM$,
        hoyMontoRecuperadoBase: hoyRM$,
        hoyNoRecuperados: hoyNoR,
        hoyMontoNoRecuperado: hoyNoRM$,
        hoyContabilizados: hoyC,
        hoyMontoContabilizado: hoyCM$,
        hoyFacturados: hoyF,
        hoyMontoFacturado: hoyFM$,
...
async function renderDashboard() {
    ...
    try {
        ...
        requestAnimationFrame(function() { animateDashboardCounters(l) })
    } catch (h) {
...
function buildDashHtml(l, venList, cliList, chartData) {
    ...
    function buildTotalKPI(ek, items, monto) {
        var s = ESTADOS[ek],
            verBtn = buildKpiAction("Ver lista", "showEstadoListModal('" + ek + "')");
        return '<div class="kpi-card kpi-flow-card kpi-state-' + ek + '"><div class="kpi-card-head"><span class="kpi-state-chip"><b>' + flowStep[ek] + '</b>' + s.nm + '</span><div class="kpi-icon">' + flowIcons[ek] + '</div></div><div class="kpi-body"><div class="kpi-main-row">' + kpiCount(items) + '<span class="kpi-unit">items</span></div><span class="kpi-lbl">Total del estado</span>' + kpiMoney(monto) + buildTotalKpiExtra(ek) + buildKpiActions(ek === "facturado" ? [verBtn, buildContabilizadosAction("total")] : [verBtn]) + '</div></div>'
    }
    function buildHoyKPI(ek, items, monto) {
        var s = ESTADOS[ek],
            verBtn = buildKpiAction("Ver lista", "showHoyListModal('" + ek + "')"),
            contBtn = ek === "facturado" ? buildContabilizadosAction("hoy") : "";
        return '<div class="kpi-card kpi-flow-card kpi-state-' + ek + ' is-today"><div class="kpi-card-head"><span class="kpi-state-chip"><b>' + flowStep[ek] + '</b>' + s.nm + '</span><div class="kpi-icon">' + flowIcons[ek] + '</div></div><div class="kpi-body"><div class="kpi-main-row">' + kpiCount(items) + '<span class="kpi-unit">items</span></div><span class="kpi-lbl">Movimiento de hoy</span>' + kpiMoney(monto) + buildHoyKpiExtra(ek) + buildKpiActions([verBtn, contBtn]) + '</div></div>'
    }
    ...
    var totalKPIs = flow.map(function(ek) { return buildTotalKPI(ek, l[ek] || 0, l["monto" + ekToKey("", ek)] || 0) }).join("") + buildAggregateKPI("Pedidos recuperados total", l.recuperadoFlujo || 0, l.montoRecuperadoFlujo || 0, false);
    var hoyKPIs = flow.map(function(ek) { return buildHoyKPI(ek, l["hoy" + ekToKey("", ek) + "s"] || 0, l["hoyMonto" + ekToKey("", ek)] || 0) }).join("") + buildAggregateKPI("Recuperados hoy", l.hoyRecuperadoFlujo || 0, l.hoyMontoRecuperadoFlujo || 0, true);
...
function buildHoyListHtml(items, estado, scope, totalCount) {
    ...
    var iconMap = {
        ...
        contabilizado: '<polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/>',
        facturado: '<polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/>'
    };
    ...
    var rows = items.map(function(w, i) {
        ...
        var motivo = estado === "no_recuperado" ? '<span class="status-badge" style="background:var(--status-error-bg);color:var(--status-error);white-space:normal;text-align:left;line-height:1.2">' + esc(getIncidenciaName(w.incidencia) || w.motivo_rechazo || w.descr_mot_rech || "-") + '</span>' : '<span style="color:var(--text-light)">—</span>';
        ...
        return '<tr style="animation-delay:' + (i * 0.04) + 's">' + fechaCell + gestorCell + '<td>' + esc(w.nombre || "") + '</td><td>' + copyChip(w.material || "-") + '</td><td>' + copyChip(w.doc_vtas || "-") + facturaCell + '<td style="text-align:left;max-width:200px">' + (estado === "no_recuperado" ? motivo : buildEstadoBadge(w.estado || estado)) + '</td><td>' + fmtGs(w.total_importe) + '</td></tr>'
    }).join("");
...
function buildRecuperadosFlujoListHtml(items, scope, totalCount) {
    ...
    var rows = items.map(function(w, i) {
        ...
        return '<tr style="animation-delay:' + (i * 0.04) + 's"><td><span class="hoy-status-dot ' + estado + '"></span>' + fmtDate(flujoFecha(w)) + '</td><td>' + esc(w.nombre || "") + '</td><td>' + copyChip(w.material || "-") + '</td><td>' + copyChip(w.doc_vtas || "-") + '</td><td style="text-align:left">' + buildEstadoBadge(estado) + '</td><td>' + copyChip(w.factura || "-") + '</td><td>' + fmtGs(w.total_importe) + '</td></tr>'
    }).join("");
    ...
    return '<div class="hoy-modal-overlay" id="hoyModalOverlay" onclick="if(event.target===this)closeHoyListModal()"><div class="hoy-modal-box"><div class="hoy-modal-head"><div class="hoy-modal-head-left"><div class="hoy-modal-head-icon recuperado_total"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></div><div class="hoy-modal-head-text"><h3>' + title + '</h3><p>' + modalSub + '</p></div></div><button class="hoy-modal-close" onclick="closeHoyListModal()"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button></div><div class="hoy-modal-body">' + (items.length ? '<table class="hoy-modal-table"><thead><tr>' + headerCells + '</tr></thead><tbody>' + rows + '</tbody></table>' : emptyHtml) + '</div><div class="hoy-modal-footer"><span>Total: <strong class="hoy-modal-count">' + totalItems + ' item' + (totalItems !== 1 ? "s" : "") + '</strong>' + footerExtra + '</span><button class="hoy-modal-close-btn" onclick="closeHoyListModal()">Cerrar</button></div></div></div>'
}
...
async function renderItemsView() {
    ...
    try {
        ...
        try {
            var sup = getSupabase();
            var { data: stockRows } = await sup.from("stock").select("*");
            var stockMap = {};
            if (stockRows) {
        ...
        itemsState.filtered = [...itemsState.all], itemsState.page = 0, l.innerHTML = buildItemsHtml();
...
function renderItemsTable() {
    ...
    k.forEach(function(w) {
        ...
        var stockClass = (+(w.stock_ldal || 0)) >= (+(w.cantidad_pedido || 0)) ? 'stock-ok' : 'stock-low';
    ...
    if (foot) { foot.style.display = ""; foot.innerHTML = '<tr class="totals-row"><td colspan="3" class="totals-label">TOTALES</td><td style="font-weight:800;color:var(--brand-dark);font-size:12px">' + fmtInt(totItems) + ' items</td><td></td><td></td><td></td><td></td><td class="pro-monto-cell" style="font-weight:800;font-size:13px">' + fmtGs(totMonto) + '</td><td></td></tr>' }
...
function applyItemsFilters() {
    ...
    filtered.sort(sortByVendorMonto);
...
async function renderRecuperadosView() {
    ...
    try {
        var h = await db_getItems({ estado: "recuperado", limit: 0 });
        var h2 = await db_getItems({ estado: "contabilizado", limit: 0 });
        var h3 = await db_getItems({ estado: "facturado", limit: 0 });
        var allRecup = (h.data || []).concat(h2.data || []).concat(h3.data || []);
        recupState.all = allRecup, recupState.filtered = [].concat(recupState.all), recupState.page = 0, l.innerHTML = buildRecupHtml();
...
function renderRecupTable() {
    ...
    updateTopBrand("Items Recuperados", getRowsDateRangeLabel(l, function(w) {
        return w.fecha_carga
    }));
    if (I.innerHTML = `<strong>${l.length} items procesados</strong>${l.length!==recupState.all.length?` <span style="opacity:0.6">(filtrados de ${recupState.all.length})</span>`:""}`, !l.length) {
        E.innerHTML = '<tr><td colspan="9"><div class="empty-state">No se encontraron items procesados</div></td></tr>', $.innerHTML = "";
        return
    }
    ...
    pageRows.forEach(w => {
      ...
      <td class="qty-cell">${fmtInt(w.cantidad_pedido)}</td>
      <td class="pro-monto-cell">${fmtGs(w.total_importe)}</td>
      <td class="pro-actions-cell">${buildEstadoBadge(w.estado)}</td>
    </tr>`
    });
...
function applyRecupFilters() {
    ...
    recupState.filtered = recupState.all.filter(function(I) {
        ...
        return !0
    }), recupState.filtered.sort(sortByVendorMonto), recupState._vendorTotals = buildVendorTotals(recupState.filtered), renderRecupTable()
}
...
async function startImport(l) {
    ...
    try {
        ...
        if (valErrors.length > 0) {
            ...
            if (!confirmed) { setImportProgress(-1); h.style.display = "none"; E && (E.style.display = "block"); I && (I.style.display = "inline-flex"); return }
        }
        ...
        g.onload = function(b) {
            try {
                ...
                for (var L = headerRow + 1; L < I.length; L++) {
                    ...
                    var rawFecha = String(v[0] || "").trim(), rawDoc = String(v[5] || "").trim(), rawMat = String(v[6] || "").trim(), rawCliente = String(v[4] || "").trim(), rawMonto = v[18];
                    ...
                    var U = null;
                    if (rawFecha) { if (typeof v[0] == "number") { var F = new Date(Math.round((v[0] - 25569) * 86400 * 1e3)); U = F.toISOString() } else { var D = rawFecha.split("/"); if (D.length === 3) { var x = new Date(D[2], D[1] - 1, D[0]); U = x.toISOString() } } }
                    valid.push({
                        fecha_carga: U, vendedor_externo: String(v[1] || "").trim() || null, vendedor_interno: String(v[2] || "").trim() || null,
                        solic: String(v[3] || "").trim() || null, nombre: rawCliente || null, doc_vtas: rawDoc || null, material: rawMat || null,
                        denominacion: String(v[7] || "").trim() || null, status: String(v[8] || "").trim() || null, motiv_rech: String(v[9] || "").trim() || null,
                        descr_mot_rech: String(v[10] || "").trim() || null, almacen: String(v[11] || "").trim() || null, fecha_recibido: Ye(v[12]),
                        cond_exp: String(v[13] || "").trim() || null, presentacion: String(v[14] || "").trim() || null, cantidad_recibida: le(v[15]),
                        cantidad_pedido: le(v[16]), cant_fact: le(v[17]),
                        total_importe: le(v[18]), estado: "pendiente"
                    })
                }
                ...
                if (stockSheet) {
                    ...
                    var stockHeaderRow = 0;
                    for (var si = 0; si < stockRows.length; si++) {
                        var srow = stockRows[si];
                        if (srow[0] && !String(srow[0]).match(/^LA[\dA-Z-]+$/i) && String(srow[0]).length > 5) { stockHeaderRow = si + 1; break }
                    }
                    for (var si = stockHeaderRow; si < stockRows.length; si++) {
                        var srow = stockRows[si];
                        var matCode = String(srow[0] || "").trim();
                        if (!matCode) continue;
                        var desc = String(srow[1] || "").trim() || null;
                        var alm = String(srow[2] || "").trim() || "VAR";
                        var cant = le(srow[3]);
                        stockData.push({ material: matCode, descripcion: desc, almacen: alm, cantidad: cant });
                    }
                }
    ...
    async function l() {
        ...
        if (h) {
            ...
            if (d) h.classList.add("hide"), h.style.opacity = "0", setTimeout(function() {
                h.style.display = "none"
            }, 400), document.getElementById("loginOverlay").style.display = "none", document.getElementById("appMain").style.display = "flex", window.currentUser = d, document.getElementById("sidebarUserName").textContent = d.nombre, document.getElementById("sidebarUserRole").textContent = ((g = ROLES[d.rol]) == null ? void 0 : g.nm) || d.rol, updateSidebarAvatar(d.nombre),             d.rol === "admin" && (document.getElementById("navAdminTitle").style.display = "block", document.getElementById("btnAdminUsers").style.display = "flex"), (d.rol === "admin" || d.rol === "operario") && (document.getElementById("btnImport").style.display = "flex"), window.renderDashboard(), startNotifPolling(!0);
            else {
...
function applyIncidFilters() {
    ...
    incidState.filtered = incidState.all.filter(function(I) {
        ...
        return !0
    }), incidState.filtered.sort(sortByVendorMonto), incidState._vendorTotals = buildVendorTotals(incidState.filtered), renderIncidTable()
}
...
function openDateModal(tab) {
    ...
    const tabNames = { items: 'Items', dash: 'Dashboard', recup: 'Recuperados', incid: 'Incidencias' };
