...
async function db_importItems(items, sinStockMap) {
    requireAuth();
    ...
    try {
        ...
        // pendienteMap: doc_vtas|material|solic → solo registros pendientes activos
        // Si ya existe como pendiente → skip. Si existe en otro estado → es un pedido nuevo, insertar.
        var pendienteMap = {};
        allExisting.forEach(function(r) {
            if (r.estado === "pendiente") {
                var pk = (r.doc_vtas || "") + "|" + (r.material || "") + "|" + (r.solic || "");
                pendienteMap[pk] = true;
            }
        });
        ...
        var toInsert = [], toMarkBorrado = [];
        items.forEach(function(x) {
            var pk = (x.doc_vtas || "") + "|" + (x.material || "") + "|" + (x.solic || "");
            // Check total stock across all warehouses from import stock sheet
            var matTotalStock = (sinStockMap && sinStockMap.hasOwnProperty(x.material))
                ? sinStockMap[x.material] : null;
            var isZeroStock = (matTotalStock !== null && matTotalStock <= 0);

            if (pendienteMap[pk]) {
                result.updated++;          // ya existe como pendiente activo → skip
            } else if (sinStockExistingMap[pk] && (isZeroStock || matTotalStock === null)) {
                result.updated++;          // ya es sin_stock y sigue sin stock → skip
            } else {
                var itemToInsert = x;
                if (isZeroStock) {
                    // Stock = 0 en todos los almacenes → clasificar directo como sin_stock
                    itemToInsert = Object.assign({}, x, {
                        estado: "sin_stock",
                        fecha_gestion: new Date().toISOString()
                    });
                }
                toInsert.push(itemToInsert);
                // Si existía como gestionado → marcar como items_borrado (fue re-eliminado en SAP)
                if (managedMap[pk]) {
                    managedMap[pk].forEach(function(id) { toMarkBorrado.push(id); });
                }
            }
        });

        // Actualizar registros re-eliminados a estado items_borrado
        if (toMarkBorrado.length > 0) {
            var MB_CHUNK = 100;
            for (var mi = 0; mi < toMarkBorrado.length; mi += MB_CHUNK) {
                var mbChunk = toMarkBorrado.slice(mi, mi + MB_CHUNK);
                await supabase.from("items_borrados").update({ estado: "items_borrado" }).in("id", mbChunk);
            }
            result.updated += toMarkBorrado.length;
        }
...
function buildDashHtml(l, venList, cliList, chartData) {
    ...
    function buildTotalKpiExtra(ek) {
        ...
        if (ek === "no_recuperado" && (l.itemsBorrado > 0 || l.sinStock > 0)) {
            var lines = [buildKpiFlowLine("No recuperado", l.no_recuperado, l.montoNoRecuperado)];
            if (l.itemsBorrado > 0) lines.push(buildKpiFlowLine("Re-borrados SAP", l.itemsBorrado, l.montoItemsBorrado, "is-pending"));
            if (l.sinStock > 0) lines.push(buildKpiFlowLine("Sin Stock", l.sinStock, l.montoSinStock, "is-pending"));
            return buildKpiFlowMeta(lines);
        }
        return ""
    }
    function buildHoyKpiExtra(ek) {
        ...
        if (ek === "no_recuperado" && (l.hoyItemsBorrados > 0 || l.hoySinStock > 0)) {
            var lines = [buildKpiFlowLine("No recuperado hoy", l.hoyNoRecuperados, l.hoyMontoNoRecuperado)];
            if (l.hoyItemsBorrados > 0) lines.push(buildKpiFlowLine("Re-borrados SAP hoy", l.hoyItemsBorrados, l.hoyMontoItemsBorrado, "is-pending"));
            if (l.hoySinStock > 0) lines.push(buildKpiFlowLine("Sin Stock hoy", l.hoySinStock, l.hoyMontoSinStock, "is-pending"));
            return buildKpiFlowMeta(lines);
        }
        return ""
    }
...
function buildHoyListHtml(items, estado, scope, totalCount) {
    ...
    var rows = items.map(function(w, i) {
        ...
        return '<tr data-fecha="' + (estado === "facturado" ? (w.fecha_fact||"") : (w.fecha_gestion||"")) + '" data-monto="' + (Number(w.total_importe)||0) + '" data-search="' + esc((w.nombre||"") + " " + (w.material||"") + " " + (w.doc_vtas||"")) + '" style="animation-delay:' + (i * 0.04) + 's">' + fechaCell + gestorCell + '<td>' + esc(w.nombre || "") + '</td><td>' + copyChip(w.material || "-") + '</td><td>' + copyChip(w.doc_vtas || "-") + facturaCell + '<td style="text-align:left;max-width:200px">' + (estado === "no_recuperado" ? motivo : buildEstadoBadge(w.estado || estado)) + '</td><td>' + fmtGs(w.total_importe) + '</td></tr>'
    }).join("");
...
function filterHoyModal(input) {
    ...
    rows.forEach(function(r) {
        var show = !term || (r.getAttribute("data-search") || "").toLowerCase().indexOf(term) !== -1;
...
async function startImport(l) {
    ...
    try {
        ...
        // Build material → total stock map for sin_stock auto-classification during this import
        var importSinStockMap = {};
        if (stockData.length > 0) {
            var _stockNorm = normalizeStockData(stockData);
            _stockNorm.rows.forEach(function(row) {
                var mat = row.material;
                if (!importSinStockMap.hasOwnProperty(mat)) importSinStockMap[mat] = 0;
                importSinStockMap[mat] += (row.cantidad || 0);
            });
        }
        if (stockData.length > 0) {
        ...
        for (var i = 0; i < total; i += BATCH * CONC) {
            ...
            for (var j = 0; j < CONC && i + j * BATCH < total; j++) {
                ...
                batchGroup.push(db_importItems(chunk, importSinStockMap))
            }
