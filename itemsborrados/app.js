...
async function db_importItems(items, sinStockMap) {
    ...
    try {
        ...
        var toInsert = [], toMarkBorradoByKey = {}, toReclassifyPendienteSinStock = [];
        items.forEach(function(x) {
            ...
            if (pendienteMap[pk] !== undefined) {
                // Item already exists as pendiente. If stock confirmed 0 now → reclassify to sin_stock.
                if (isZeroStock) {
                    toReclassifyPendienteSinStock.push(pendienteMap[pk]);
                }
                result.updated++;
        ...
        // Reclasificar pendientes con stock 0 confirmado → sin_stock
        if (toReclassifyPendienteSinStock.length > 0) {
            var nowSS = new Date().toISOString();
            var SS_CHUNK = 100;
            for (var ssi = 0; ssi < toReclassifyPendienteSinStock.length; ssi += SS_CHUNK) {
                var ssBatch = toReclassifyPendienteSinStock.slice(ssi, ssi + SS_CHUNK);
                await supabase.from("items_borrados")
                    .update({ estado: "sin_stock", fecha_gestion: nowSS })
                    .in("id", ssBatch);
            }
            result.sinStock = (result.sinStock || 0) + toReclassifyPendienteSinStock.length;
        }
...
function buildHoyListHtml(items, estado, scope, totalCount) {
    ...
    var rows = items.map(function(w, i) {
        ...
        var motivoKey = rowEstado === "sin_stock" ? "__sin_stock__"
            : rowEstado === "items_borrado" ? "__items_borrado__"
    ...
    if (estado === "no_recuperado" && items.length > 0) {
        ...
        items.forEach(function(w) {
            ...
            var mk = re === "sin_stock" ? "__sin_stock__" : re === "items_borrado" ? "__items_borrado__" : (getIncidenciaName(w.incidencia) || w.motivo_rechazo || w.descr_mot_rech || "Sin motivo");
            if (!_mSet[mk]) { _mSet[mk] = true; _mKeys.push(mk); }
        });
        if (_mKeys.length > 1) {
            var _opts = _mKeys.map(function(mk) {
                var lbl = mk === "__sin_stock__" ? "Sin Stock" : mk === "__items_borrado__" ? "Reborrado" : mk;
                return '<option value="' + esc(mk) + '">' + esc(lbl) + '</option>';
            }).join("");
    ...
    try {
        ...
        try {
            ...
            itemsState.all.forEach(function(item) {
                ...
                item.stock_total = Object.values(matStock).reduce(function(s, v) { return s + (Number(v) || 0); }, 0);
            });
        } catch (stockErr) {
...
function renderItemsTable() {
    ...
    if (foot) { foot.style.display = ""; foot.innerHTML = '<tr class="totals-row"><td colspan="3" class="totals-label">TOTALES</td><td style="font-weight:800;color:var(--brand-dark);font-size:12px">' + fmtInt(totItems) + ' items</td><td></td><td></td><td></td><td></td><td></td><td></td><td class="pro-monto-cell" style="font-weight:800;font-size:13px">' + fmtGs(totMonto) + '</td><td></td></tr>' }
...
async function startImport(l) {
    ...
    try {
        ...
        if (stockData.length > 0) {
            ...
            // Sin hoja stock en el Excel → usar tabla stock de Supabase como respaldo
            try {
                var { data: _stockTable } = await getSupabase().from("stock").select("material, cantidad");
                if (_stockTable && _stockTable.length > 0) {
                    _stockTable.forEach(function(row) {
                        var mat = row.material;
                        if (!importSinStockMap.hasOwnProperty(mat)) importSinStockMap[mat] = 0;
                        importSinStockMap[mat] += (row.cantidad || 0);
                    });
                }
            } catch(e) { console.warn("No se pudo cargar stock para clasificación sin_stock:", e); }
        }
        if (stockData.length > 0) {
        ...
        g.onload = function(b) {
            try {
                ...
                if (stockSheet) {
                    ...
                    // Buscar fila de encabezado buscando palabras clave de columnas de stock en las primeras 20 filas
                    var matKw = ["material", "materi", "cod.", "código", "codigo"];
                    var almKw = ["almac", "deposito", "depósito", "ubicac", "storage", "lager", "loc."];
                    var cantKw = ["cant", "stock", "libre", "unrest", "total", "cantidad", "menge", "qty"];
                    for (var si = 0; si < Math.min(stockRows.length, 20); si++) {
                        var srow = stockRows[si];
                        var cells = srow.map(function(c) { return String(c || "").trim().toLowerCase(); });
                        var foundMat = -1, foundAlm = -1, foundCant = -1;
                        cells.forEach(function(c, ci) {
                            if (foundMat === -1 && matKw.some(function(k) { return c.indexOf(k) !== -1; })) foundMat = ci;
                            if (foundAlm === -1 && almKw.some(function(k) { return c.indexOf(k) !== -1; })) foundAlm = ci;
                            if (foundCant === -1 && cantKw.some(function(k) { return c.indexOf(k) !== -1; })) foundCant = ci;
                        });
                        if (foundMat !== -1 && (foundAlm !== -1 || foundCant !== -1)) {
                            stockHeaderRow = si;
                            stockColMat  = foundMat;
                            stockColDesc = foundMat + 1 < cells.length ? foundMat + 1 : 1;
                            stockColAlm  = foundAlm !== -1 ? foundAlm : stockColDesc + 1;
                            stockColCant = foundCant !== -1 ? foundCant : stockColAlm + 1;
                            break;
                        }
                    }
                    // Fallback: heurística original si no se encontró encabezado por keywords
                    if (stockHeaderRow === -1) {
                        for (var si = 0; si < stockRows.length; si++) {
                            var srow = stockRows[si];
                            if (srow[0] && !String(srow[0]).match(/^[A-Z]{1,4}[\dA-Z-]+$/i) && String(srow[0]).trim().length > 3) {
                                stockHeaderRow = si;
                                break;
                            }
                        }
