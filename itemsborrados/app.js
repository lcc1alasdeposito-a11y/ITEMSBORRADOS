...
function buildRecuperadosFlujoListHtml(items, scope, totalCount) {
    ...
    var rows = items.map(function(w, i) {
        ...
        return '<tr data-fecha="' + (flujoFecha(w)||"") + '" data-monto="' + (Number(w.total_importe)||0) + '" style="animation-delay:' + (i * 0.04) + 's"><td><span class="hoy-status-dot ' + estado + '"></span>' + fmtDate(flujoFecha(w)) + '</td><td>' + esc(w.nombre || "") + '</td><td>' + copyChip(w.material || "-") + '</td><td>' + copyChip(w.doc_vtas || "-") + '</td><td style="text-align:left">' + buildFlujoEstadoBadge(estado) + '</td><td>' + copyChip(w.factura || "-") + '</td><td>' + fmtGs(w.total_importe) + '</td></tr>'
    }).join("");
