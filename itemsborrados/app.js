...
async function startImport(l) {
    ...
    try {
        ...
        // Paso final: usar importSinStockMap (ya tiene todos los materiales del Excel en memoria)
        // y cruzar contra todos los pendientes en items_borrados
        if (Object.keys(importSinStockMap).length > 0) {
            b.textContent = "Verificando items sin stock...";
            try {
                // Traer todos los pendientes con paginación (Supabase limita a 1000 por query)
                ...
                if (allPendientes.length > 0) {
                    var sinStockIds = allPendientes.filter(function(item) {
                        ...
                        return importSinStockMap.hasOwnProperty(mat) && importSinStockMap[mat] <= 0;
