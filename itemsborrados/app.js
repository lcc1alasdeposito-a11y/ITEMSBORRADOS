...
async function startImport(l) {
    ...
    try {
        ...
        // Paso final: traer pendientes, sacar materiales únicos, consultar solo esos en stock
        b.textContent = "Verificando items sin stock...";
        try {
            // 1. Traer todos los pendientes (paginado)
            var allPendientes = [], pOffset = 0, pBatch = 1000;
            while (true) {
                var { data: pChunk, error: pErr } = await getSupabase()
                    .from("items_borrados").select("id, material")
                    .eq("estado", "pendiente")
                    .range(pOffset, pOffset + pBatch - 1);
                if (pErr) throw pErr;
                if (!pChunk || pChunk.length === 0) break;
                allPendientes = allPendientes.concat(pChunk);
                if (pChunk.length < pBatch) break;
                pOffset += pBatch;
            }
            if (allPendientes.length > 0) {
                // 2. Sacar materiales únicos de los pendientes
                var matsUnicos = [];
                var matsSet = {};
                allPendientes.forEach(function(p) {
                    var mat = String(p.material || "").trim();
                    if (mat && !matsSet[mat]) { matsSet[mat] = true; matsUnicos.push(mat); }
                });
                // 3. Consultar stock solo para esos materiales (en lotes de 500)
                var stockTotalByMat = {};
                for (var mi = 0; mi < matsUnicos.length; mi += 500) {
                    var matsBatch = matsUnicos.slice(mi, mi + 500);
                    var { data: sRows, error: sErr } = await getSupabase()
                        .from("stock").select("material, cantidad")
                        .in("material", matsBatch);
                    if (sErr) throw sErr;
                    (sRows || []).forEach(function(r) {
                        var mat = String(r.material || "").trim();
                        if (!stockTotalByMat[mat]) stockTotalByMat[mat] = 0;
                        stockTotalByMat[mat] += (Number(r.cantidad) || 0);
                    });
                }
                // 4. Los pendientes cuyo material está en stock con total = 0 → sin_stock
                var sinStockIds = allPendientes.filter(function(item) {
                    var mat = String(item.material || "").trim();
                    return stockTotalByMat.hasOwnProperty(mat) && stockTotalByMat[mat] <= 0;
                }).map(function(item) { return item.id; });
                if (sinStockIds.length > 0) {
                    var nowSS = new Date().toISOString();
                    for (var ssi = 0; ssi < sinStockIds.length; ssi += 100) {
                        await getSupabase().from("items_borrados")
                            .update({ estado: "sin_stock", fecha_gestion: nowSS })
                            .in("id", sinStockIds.slice(ssi, ssi + 100));
                    }
                    stats.sinStock = sinStockIds.length;
                }
            }
        } catch(ssErr) { console.warn("Error verificando sin_stock:", ssErr); }
