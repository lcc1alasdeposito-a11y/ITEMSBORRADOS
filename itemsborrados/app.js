    ...
    try {
        ...
        while (true) {
            var { data: page, error: err } = await supabase
                .from("items_borrados")
                .select("id, doc_vtas, fecha_carga, material, solic, estado")
                .in("doc_vtas", docList)
        ...
        // pendienteMap: doc_vtas|material|fecha_carga → only estado=pendiente (business dedup key)
        // constraintMap: doc_vtas|material|solic → any estado (prevents DB unique constraint violation)
        var pendienteMap = {}, constraintMap = {};
        allExisting.forEach(function(r) {
            var ck = (r.doc_vtas || "") + "|" + (r.material || "") + "|" + (r.solic || "");
            if (!constraintMap[ck]) constraintMap[ck] = true;
            if (r.estado === "pendiente") {
                var f = (r.fecha_carga || "").substring(0, 10);
                pendienteMap[(r.doc_vtas || "") + "|" + (r.material || "") + "|" + f] = true;
            }
        });
        ...
        items.forEach(function(x) {
            ...
            var pk = (x.doc_vtas || "") + "|" + (x.material || "") + "|" + f;
            var ck = (x.doc_vtas || "") + "|" + (x.material || "") + "|" + (x.solic || "");
            if (pendienteMap[pk]) {
                result.updated++;          // same date+doc+material already pendiente → skip
            } else if (constraintMap[ck]) {
                result.updated++;          // exists with other estado → skip until DB constraint is fixed
            } else {
                toInsert.push(x);          // truly new → insert
            }
        });
...
async function db_saveUser(l) {
    ...
    if (current.rol !== "admin") throw new Error("No autorizado");
    ...
    if (!adminPinHash) throw new Error("Ingresá tu PIN de administrador para confirmar");
...
async function db_getStats(l, desde, hasta) {
    ...
    return {
        ...
        montoPendiente: (s.montoTotal || 0) - (s.montoRecuperado || 0) - (s.montoNoRecuperado || 0) - (s.montoContabilizado || 0) - (s.montoFacturado || 0),
...
async function doLogin() {
    ...
    try {
        const hashedPin = await hashPin(b);
        const E = await db_login(g, hashedPin);
        resetLoginAttempts();
        saveSession(E), document.getElementById("loginOverlay").style.display = "none", document.getElementById("appMain").style.display = "flex", document.getElementById("sidebarUserName").textContent = E.nombre, document.getElementById("sidebarUserRole").textContent = ((k = ROLES[E.rol]) == null ? void 0 : k.nm) || E.rol, updateSidebarAvatar(E.nombre),             E.rol === "admin" && (document.getElementById("navAdminTitle").style.display = "block", document.getElementById("btnAdminUsers").style.display = "flex"), (E.rol === "admin" || E.rol === "operario") && (document.getElementById("btnImport").style.display = "flex"), renderDashboard(), startNotifPolling(!0)
    } catch (E) {
        recordFailedAttempt();
...
async function renderDashboard() {
    ...
    try {
        ...
        var venMap = {}, cliMap = {};
        var RES_ESTS = ["pendiente", "recuperado", "contabilizado", "facturado", "no_recuperado"];
...
function buildHoyListHtml(items, estado, scope, totalCount) {
    ...
    var rows = items.map(function(w, i) {
        ...
        return '<tr data-fecha="' + (estado === "facturado" ? (w.fecha_fact||"") : (w.fecha_gestion||"")) + '" data-monto="' + (Number(w.total_importe)||0) + '" style="animation-delay:' + (i * 0.04) + 's">' + fechaCell + gestorCell + '<td>' + esc(w.nombre || "") + '</td><td>' + copyChip(w.material || "-") + '</td><td>' + copyChip(w.doc_vtas || "-") + facturaCell + '<td style="text-align:left;max-width:200px">' + (estado === "no_recuperado" ? motivo : buildEstadoBadge(w.estado || estado)) + '</td><td>' + fmtGs(w.total_importe) + '</td></tr>'
    }).join("");
...
function buildItemsHtml() {
    ...
    <div class="items-table-wrap pro-table-wrap">
      <table class="items-table pro-table">
        <thead>
          <tr>
            <th>${tableSortButton("items", "fecha_carga", "Fecha Carga", itemsState)}</th>
            <th>Cliente</th>
            <th>DOC. VENTA</th>
            <th>Material</th>
            <th class="th-principal" title="Almacén Principal">${tableSortButton("items", "stock_ldal", '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="margin-bottom:-1px"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg> Stock LDAL', itemsState, "th-chip-blue")}</th>
            <th class="th-secundario">${tableSortButton("items", "stock_ldfa", "Stock LDFA", itemsState, "th-chip-gray")}</th>
            <th class="th-secundario">${tableSortButton("items", "stock_ldlq", "Stock LDLQ", itemsState, "th-chip-gray")}</th>
            <th style="text-align:right">${tableSortButton("items", "cantidad_pedido", "Cant. Pedida", itemsState)}</th>
            <th class="pro-col-monto">${tableSortButton("items", "total_importe", "Monto Total", itemsState)}</th>
...
function renderRecupTable() {
    ...
    pageRows.forEach(w => {
      ...
      <td class="qty-cell">${fmtInt(w.cantidad_pedido)}</td>
      <td class="pro-monto-cell">${fmtGs(w.total_importe)}</td>
      <td class="pro-actions-cell">${buildEstadoBadge(estado)}${estadoExtra}</td>
    </tr>`
    });
      ...
      <div class="modal-box user-form-modal" onclick="event.stopPropagation()">
        ...
        <div class="modal-body user-form-body">
          ...
          <div class="user-form-group" id="userFormPinGroup">
            ...
            <p class="user-form-hint">Solo para rol de Administrador. En edici&oacute;n pod&eacute;s dejarlo vac&iacute;o para conservar el PIN actual.</p>
          </div>

          <div class="user-form-group">
            <label class="user-form-label">Confirmar como administrador</label>
            <div class="user-form-input-wrap">
            ...
            <p class="user-form-hint">Este PIN no se guarda en el usuario; solo autoriza la operaci&oacute;n</p>
          </div>
...
function syncUserPinVisibility() {
    ...
    if (pinGroup) pinGroup.style.display = isAdmin ? '' : 'none';
    if (!isAdmin && pinInput) {
        pinInput.value = '';
    ...
    if (preview) preview.textContent = isAdmin ? 'A' : 'O';
}
...
function nuevoUsuario() {
    ...
    document.getElementById("userFormPin").placeholder = "Nuevo PIN";
...
function editarUsuario(l) {
    ...
    document.getElementById("userFormPin").value = "";
    document.getElementById("userFormPin").type = "password";
    document.getElementById("userFormPin").placeholder = "Dejar vac\u00edo para conservar";
    document.getElementById("userAdminPin").value = "";
