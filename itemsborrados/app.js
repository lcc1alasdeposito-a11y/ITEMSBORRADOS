...
async function startImport(l) {
    ...
    try {
        ...
        // Paso final: consultar stock table y pasar a sin_stock los pendientes con total = 0 en todos los almacenes
        b.textContent = "Verificando stock de items pendientes...";
        try {
            var { data: stockTableRows } = await getSupabase().from("stock").select("material, cantidad");
            if (stockTableRows && stockTableRows.length > 0) {
                // Sumar todas las cantidades por material (todos los almacenes)
                var stockTotalMap = {};
                stockTableRows.forEach(function(row) {
                    var mat = String(row.material || "").trim();
                    if (!mat) return;
                    if (!stockTotalMap.hasOwnProperty(mat)) stockTotalMap[mat] = 0;
                    stockTotalMap[mat] += (Number(row.cantidad) || 0);
                });
                // Traer todos los pendientes de items_borrados
                var { data: allPendientes } = await getSupabase()
                    .from("items_borrados")
                    .select("id, material")
                    .eq("estado", "pendiente");
                if (allPendientes && allPendientes.length > 0) {
                    var sinStockIds = allPendientes
                        .filter(function(item) {
                            var mat = String(item.material || "").trim();
                            // Solo si el material está en la tabla stock y su total es 0
                            return stockTotalMap.hasOwnProperty(mat) && stockTotalMap[mat] <= 0;
                        })
                        .map(function(item) { return item.id; });
                    if (sinStockIds.length > 0) {
                        var nowSS = new Date().toISOString();
                        var SS_CHUNK = 100;
                        for (var ssi = 0; ssi < sinStockIds.length; ssi += SS_CHUNK) {
                            var ssBatch = sinStockIds.slice(ssi, ssi + SS_CHUNK);
                            await getSupabase().from("items_borrados")
                                .update({ estado: "sin_stock", fecha_gestion: nowSS })
                                .in("id", ssBatch);
                        }
                        stats.sinStock = sinStockIds.length;
                    }
                }
            }
        } catch(ssErr) { console.warn("Error en verificación sin_stock:", ssErr); }
