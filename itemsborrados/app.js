...
async function renderDashboard() {
    ...
    var now = new Date();
    var savedMes = now.getFullYear() + '-' + String(now.getMonth() + 1).padStart(2, '0');
    writeControlValue("dashFilterMesValue", savedMes);
    var parts = savedMes.split("-");
    var y = parseInt(parts[0], 10), m = parseInt(parts[1], 10) - 1;
    var lastDay = new Date(y, m + 1, 0).getDate();
    writeControlValue("dashFilterFechaDesde", savedMes + "-01");
    writeControlValue("dashFilterFechaHasta", savedMes + "-" + String(lastDay).padStart(2, "0"));
...
function onDashMesNav(dir) {
    var savedMes = readControlValue("dashFilterMesValue");
    var parts = savedMes.split("-");
    ...
    writeControlValue("dashFilterMesValue", val);
    var desde = val + "-01";
    var lastDay = new Date(year, month + 1, 0).getDate();
    var hasta = val + "-" + String(lastDay).padStart(2, "0");
    writeControlValue("dashFilterFechaDesde", desde);
    writeControlValue("dashFilterFechaHasta", hasta);
    syncDateTriggerUI("dash");
    renderDashboard();
}
...
function buildDashHtml(l, venList, cliList, chartData) {
    ...
    var dashDesdeValue = arguments.length > 4 ? arguments[4] || "" : readControlValue("dashFilterFechaDesde"),
        ...
        now = new Date(),
        currentMesVal = readControlValue("dashFilterMesValue") || now.getFullYear() + '-' + String(now.getMonth() + 1).padStart(2, '0');
    var mesParts = currentMesVal.split('-');
    var mesYear = parseInt(mesParts[0], 10);
    var mesMonth = parseInt(mesParts[1], 10) - 1;
    var mesLabel = monthNames[mesMonth] + ' ' + mesYear;
