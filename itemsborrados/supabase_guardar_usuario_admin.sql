...
begin
  ...
  if not found then
    raise exception 'PIN incorrecto o usuario no autorizado'
      using errcode = '42501';
  end if;
  ...
  if p_rol not in ('admin', 'supervisor', 'operario') then
  ...
  if v_actor.rol = 'supervisor' then
    if p_id <> v_actor.id then
      raise exception 'El supervisor solo puede editar su propio PIN'
        using errcode = '42501';
    end if;

    if coalesce(p_pin_hash, '') = '' then
      raise exception 'El nuevo PIN es obligatorio';
    end if;
  ...
  if coalesce(p_pin_hash, '') <> '' then
    ...
    v_pin := v_target.pin;
  elsif p_rol in ('admin', 'supervisor') then
    raise exception 'El PIN es obligatorio para Administrador o Supervisor';
  else
    v_pin := '';
  end if;
...
as $$
  select
    ...
    u.rol,
    u.activo,
    coalesce(nullif(trim(u.pin), ''), '') <> '' as tiene_pin
